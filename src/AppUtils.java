...
public class AppUtils {
    ...
    public static ArrayList<Coche> cargarCoches(String filename) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f))) {
            ...
            JOptionPane.showMessageDialog(null, "Error a cargar el catálogo de los coches " + e.getMessage());
            return new ArrayList<>();
        }
    }

    public static void guardarCoches(ArrayList<Coche> catalogo, String filename) {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(filename))) {
            oos.writeObject(catalogo);
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "Error cargando coches: " + e.getMessage());
        }
    }
    ...
    public static void calcularRentabilidad(ArrayList<Coche> catalogo) {
        if (catalogo.isEmpty()) {
            JOptionPane.showMessageDialog(null, "No hay ningún coche en la lista");
            return;
        }
        ...
        if (indice != -1){
            Coche currentCar = catalogo.get(indice); 
            JOptionPane.showMessageDialog(null, currentCar.taxesSpain.toString() 
            + "\n" + currentCar.taxesGermany.toString() + "\n");
            currentCar.contarLaRentabilidad();
        } else {
            return;
        }
    }
